...
function HomePage({ isCardDetailPage = false, customLoadButton = "Load More", customTitle = "Recent Scripts", cardsData}){
  ...
  const [isOpen, setOpen] = useState(false);
    ...
    const handleLoadMore = () => {
      setVisibleCardsCount(prevCount => prevCount + CARDS_PER_LOAD);
    };

    const showLoadMoreButton = visibleCardsCount < cardsData.length;

    return(
    <>
    <main className='main container'>
        <div className="main__top">
          <h1 className="main__top-title">{ customTitle }</h1>
          { !isCardDetailPage && (
            <div>
        ...
        <section className="main__cards">
           {cardsData.slice(0, visibleCardsCount).map(cardInfo=>{ //??
            return(
              <Card
                key={cardInfo.id}
                id={cardInfo.id}
        ...
        {!isCardDetailPage && showLoadMoreButton && (
          <button className="main__load-button" onClick={handleLoadMore}>
            {customLoadButton}
          </button>
)}
        {isCardDetailPage && 
        (
        <Link to="/">
          <button className="main__load-button">
            { customLoadButton }
          </button>
        </Link>)}
     </main>
...
<path d="M12 10V20M12 20L9.5 17.5M12 20L14.5 17.5" stroke="#currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
<path fill-rule="evenodd" clip-rule="evenodd" d="M6.3218 7.05726C7.12925 4.69709 9.36551 3 12 3C14.6345 3 16.8708 4.69709 17.6782 7.05726C19.5643 7.37938 21 9.02203 21 11C21 13.2091 19.2091 15 17 15H16C15.4477 15 15 14.5523 15 14C15 13.4477 15.4477 13 16 13H17C18.1046 13 19 12.1046 19 11C19 9.89543 18.1046 9 17 9C16.9776 9 16.9552 9.00037 16.9329 9.0011C16.4452 9.01702 16.0172 8.67854 15.9202 8.20023C15.5502 6.37422 13.9345 5 12 5C10.0655 5 8.44979 6.37422 8.07977 8.20023C7.98284 8.67854 7.55482 9.01702 7.06706 9.0011C7.04476 9.00037 7.02241 9 7 9C5.89543 9 5 9.89543 5 11C5 12.1046 5.89543 13 7 13H8C8.55228 13 9 13.4477 9 14C9 14.5523 8.55228 15 8 15H7C4.79086 15 3 13.2091 3 11C3 9.02203 4.43567 7.37938 6.3218 7.05726Z" fill="currentColor"/>
</svg>
          </div>
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M12 23C18.0751 23 23 18.0751 23 12C23 5.92487 18.0751 1 12 1C5.92487 1 1 5.92487 1 12C1 18.0751 5.92487 23 12 23ZM12 20.9932C7.03321 20.9932 3.00683 16.9668 3.00683 12C3.00683 7.03321 7.03321 3.00683 12 3.00683C16.9668 3.00683 20.9932 7.03321 20.9932 12C20.9932 16.9668 16.9668 20.9932 12 20.9932Z" fill="currentColor"/>
      ...
      <div className="stat__inTouch">
        ...
        <button>Send Message <svg fill="currentColor" width="800px" height="800px" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
