...
function CardDetail(){
    ...
    useEffect(()=>{
        const fetchCardDetails = async() =>{
            try{
                ...
                const response = await fetch(`/api/cards/${id}`); // Предполагаем, что у вас есть эндпоинт для получения одной карточки
                if (!response.ok) {
    ...
    const { imgPath, title, description, downloadCount, uploadedAt } = cardData; // Изменены названия полей согласно Go модели

    // Форматируем дату, если она приходит как строка или объект Date
    const formattedUploadDate = uploadedAt ? new Date(uploadedAt).toLocaleDateString() : ''; // Пример форматирования
    
    return(
        <>
        <div className="cardInfo container">
            <div className="cardInfo__image">
                <img src={imgPath} alt={title || "Изображение карточки"} /> 
            </div>
